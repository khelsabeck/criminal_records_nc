'''
This module has one class, dumbwaiter, which serves as the context object for the habitualmachine FSM. 
Dumbwaiter has a list of habitual_convictions (convictions that have been validated through the FSM as counting toward hab status). 
...
Dumbwaiter has a bool hab_eligible if the defendant is habitual eligible, and a date_eligible for the date at which they became eligible.
Dumbwaiter also has a method taking a new conviction and returning whether it is eligible to be raised to habitual status.
'''
...
class Dumbwaiter:
    ...
    def __init__(self, birthdate):
        ...
        self.ran = False                # when the program has run, this should be set as True
    ...
    def set_date_eligible(self, conviction_date):
        '''This sets the date at which the defendant became habitual-eligible. This method takes the conviction date and checks its conviction date. If it is
